...
def unpack_bnd(content):
    ...
    master_offset = 0x0C
    (magic_flag, num_of_records, filename_end_offset) = struct.unpack_from(
        "<III", content, offset=master_offset
    )
    ...
    for _ in range(num_of_records):
        if magic_flag == 0x74 or magic_flag == 0x54:
            (
                ...
                filename_offset,
                dummy_filedata_size,
            ) = struct.unpack_from("<IIIIII", content, offset=master_offset)
            master_offset += struct.calcsize("<IIIIII")
            if filedata_size != dummy_filedata_size:
                raise ValueError(
                    "File has malformed record structure. File data size "
                    + str(filedata_size)
                    + " does not match dummy file data size "
            ...
            (
                ...
                file_id,
                filename_offset,
            ) = struct.unpack_from("<IIIII", content, offset=master_offset)
            master_offset += struct.calcsize("<IIIII")

        if record_sep != 0x40:
            raise ValueError(
                "File has malformed record structure. Record"
                + " has unknown record separator "
                + hex(record_sep)
            )
...
def repack_bnd(content_list):
    ...
    HEADER = b"BND307D7R6\x00\x00" + struct.pack(
        "<IIIII", 0x74, num_of_records, filename_end_offset, 0, 0
    )
