...
class ItemTable:
    ...
    def has_room_at_location_for_itemlotpart(self, itemlotpart, loc_id, item_list):
        ...
        proposed_new_itemlot = (
            self.table[loc_id]
            + [itemlotpart]
            + [item_list[i] for i in itemlotpart.follow_items]
        )
        proposed_flags = [i.flag for i in proposed_new_itemlot]
        has_replaceable_flag = (
            len([i.flag for i in proposed_new_itemlot if not i.needs_flag]) > 0
        )

        if (
            location.has_flag != -1
            and location.has_flag not in proposed_flags
            and not has_replaceable_flag
        ):
    ...
    def reduce_flag_usage(self, loc_id):
        ...
        for flag1, flag2 in itertools.product(sorted_flags, repeat=2):
            if (
                flag1 < flag2
                and not (
                    flag_usage_dict[flag1]["req"] and flag_usage_dict[flag2]["req"]
                )
                and flag_usage_dict[flag1]["length"] + flag_usage_dict[flag2]["length"]
                <= MAX_FLAG_GROUP_LEN
            ):
                dominant_flag = flag1
    ...
    def merge_flags(self, loc_id):
        ...
        for flag in flag_usage_dict:
            if flag_usage_dict[flag]["length"] == 0:
                ...
                dummy = item_s.ItemLotPart(
                    ...
                    2,
                    [item_s.ItemLotEntry(item_s.ITEM_TYPE.ITEM, 330)],
                    flag=flag,
                    needs_flag=True,
                )
        ...
        self.table[loc_id].sort(
            key=lambda item: (not flag_usage_dict[item.flag]["at_start"])
        )
    ...
    def fix_pickup_flags(self):
        ...
        for loc_id in sorted(self.location_dict):
            ...
            if loc.diff in [
                loc_s.LOC_DIF.EASY,
                loc_s.LOC_DIF.MEDIUM,
                loc_s.LOC_DIF.HARD,
                loc_s.LOC_DIF.STARTING_ITEM,
            ]:
        ...
        for loc_id in self.location_dict:
            ...
            for itemlotpart in itemlotparts:
                ...
                for entry in itemlotpart.items:
                    item = ilp.ItemLotItem(
                        CATEGORY_TRANSLATION[entry.item_type],
                        entry.item_id,
                        entry.count,
                        entry.rate,
                        0,
                ...
                if loc.has_cumul_flag and len(items) > 0:
                    ...
                    empty = ilp.ItemLotItem(
                        ilp.ItemLotItemType.NONE,
                        0,
                        0,
                        loc.has_cumul_flag.chance_denom
                        - loc.has_cumul_flag.chance_numer,
                        0,
                        0,
                        True,
                        False,
            ...
            if len(self.table[loc_id]) > 0 and len(self.table[loc_id][0].items) > 0:
                ...
                for link_loc_id in [loc_id] + loc.linked_locations:
                    if link_loc_id in self.shop_dict:
                        ...
                        lineup = slp.ShopLineup(
                            shop_data.shop_id,
                            CATEGORY_TRANSLATION[item.item_type],
                            item.item_id,
                            shop_data.cost,
                            item.count,
                            itemlotpart.flag,
    ...
    def build_cheatsheet(self, show_event_flags=False):
        ...
        for loc_id in sorted(list(self.location_dict.keys())):
            ...
            if loc.diff in [
                ...
                location_string = (
                    item_lot_formatter.format_item_table_entry_as_human_readable(
                        loc,
                        self.table[loc_id],
                        cost=cost,
                        show_event_flags=show_event_flags,
                    )
                )
