...
DESC_DICT = {
    "diff": {
        rngopts.RandOptDifficulty.EASY: "* Perfectly fair. Items have an equal chance to be placed anywhere.\n",
    ...
    "key_diff": {
        rngopts.RandOptKeyDifficulty.LEAVE_ALONE: (
            "* Key items are placed in their usual locations.\n"
        ...
        rngopts.RandOptKeyDifficulty.RANDOMIZE: (
            "* Key items are shuffled into the item pool and placed in random locations.\n"
    ...
    "souls_diff": {
        rngopts.RandOptSoulItemsDifficulty.SHUFFLE: "* Soul items are shuffled into the item pool like other items.\n\n",
    ...
    "start_items": {
        rngopts.RandOptStartItemsDifficulty.SHIELD_AND_1H: (
            "* Player starts with random class-usable (L) shield & (R) weapon.\n"
    ...
    "use_lord_souls": {
        True: "* The 4 Lord Souls ARE included in the randomized keys.\n   Difficulty ranges from much easier to much harder.",
...
class MainGUI:
    def __init__(self):
        ...
        self.seed_entry = tk.Entry(
            self.root, font="TkFixedFont", textvariable=self.seed_string, width=70
        )
        self.entry_state = self.add_placeholder_to(
            self.seed_entry, "Type a seed (or leave blank for a random seed)"
        )
        self.seed_entry.grid(
            row=0, column=1, columnspan=2, ipady=2, ipadx=1, padx=2, sticky="EW"
        )
        tk.Label(self.root, text="Made by HotPocketRemix      ").grid(
            row=0, column=3, columnspan=2, sticky="E", padx=2
        )
        self.sellout_button = tk.Button(
            self.root,
            text="$",
            bg="pale goldenrod",
            padx=2,
            pady=2,
            command=self.lift_sellout_area,
        )
        self.sellout_button.grid(row=0, column=4, padx=2, sticky="E")

        tk.Label(self.root, text="Dark Souls Game Version:").grid(
            row=1, column=0, columnspan=2, sticky="W", padx=2, ipady=1
        )
        self.game_version = tk.StringVar()
        self.game_version_menu = ttk.Combobox(
            self.root,
            textvariable=self.game_version,
            state="readonly",
            values=[
                rngopts.RandOptGameVersion.PTDE,
                rngopts.RandOptGameVersion.REMASTERED,
            ],
            style="GameVersion.TCombobox",
        )
        self.style.map(
            "Highlight.GameVersion.TCombobox",
            fieldbackground=[("readonly", "light salmon")],
        )
        ...
        self.game_version_menu.bind(
            "<<ComboboxSelected>>", lambda _: self.update_game_version()
        )
        self.game_version_menu.config(width=30)
        self.game_version_menu.grid(row=1, column=2, sticky="EW", padx=2)

        self.msg_area = tk.Text(
            self.root,
            width=76,
            height=19,
            state="disabled",
        ...
        self.msg_quit_button = tk.Button(
            self.root, text="Quit", command=self.quit_button
        )
        self.msg_quit_button.grid(row=9, column=1, columnspan=2, rowspan=2)
        self.msg_continue_button = tk.Button(
            self.root, text="Continue", command=self.continue_button
        )
        ...
        self.setup_hover_events(
            self.diff_rbutton1, {"diff": rngopts.RandOptDifficulty.EASY}
        )
        self.setup_hover_events(
            self.diff_rbutton2, {"diff": rngopts.RandOptDifficulty.MEDIUM}
        )
        self.setup_hover_events(
            self.diff_rbutton3, {"diff": rngopts.RandOptDifficulty.HARD}
        )
        ...
        self.setup_hover_events(
            self.key_diff_rbutton1,
            {
                "key_diff": rngopts.RandOptKeyDifficulty.LEAVE_ALONE,
                "use_lv": False,
                "use_lord_souls": False,
            },
        )
        self.setup_hover_events(
            self.key_diff_rbutton2, {"key_diff": rngopts.RandOptKeyDifficulty.RANDOMIZE}
        )
        self.setup_hover_events(
            self.key_diff_rbutton3, {"key_diff": rngopts.RandOptKeyDifficulty.RACE_MODE}
        )
        self.setup_hover_events(
            self.key_diff_rbutton4,
            {
                "key_diff": rngopts.RandOptKeyDifficulty.SPEEDRUN_MODE,
                "diff": rngopts.RandOptDifficulty.EASY,
            },
        )
        ...
        self.setup_hover_events(
            self.soul_diff_rbutton1,
            {"souls_diff": rngopts.RandOptSoulItemsDifficulty.SHUFFLE},
        )
        self.setup_hover_events(
            self.soul_diff_rbutton2,
            {"souls_diff": rngopts.RandOptSoulItemsDifficulty.CONSUMABLE},
        )
        self.setup_hover_events(
            self.soul_diff_rbutton3,
            {"souls_diff": rngopts.RandOptSoulItemsDifficulty.TRANSPOSE},
        )
        ...
        self.setup_hover_events(
            self.start_items_rbutton1,
            {"start_items": rngopts.RandOptStartItemsDifficulty.SHIELD_AND_1H},
        )
        self.setup_hover_events(
            self.start_items_rbutton2,
            {"start_items": rngopts.RandOptStartItemsDifficulty.SHIELD_AND_2H},
        )
        self.setup_hover_events(
            self.start_items_rbutton3,
            {"start_items": rngopts.RandOptStartItemsDifficulty.COMBINED_POOL_AND_2H},
        )

        self.fashion_bool = tk.BooleanVar()
        self.fashion_bool.set(True)
        self.fashion_bool.trace("w", lambda name, index, mode: self.update())
        self.fashion_check = tk.Checkbutton(
            self.root,
            text="Fashion Souls",
            variable=self.fashion_bool,
            onvalue=True,
            offvalue=False,
            padx=2,
            width=20,
        ...
        self.npc_armor_check = tk.Checkbutton(
            self.root,
            text="Laundromat Mixup",
            variable=self.npc_armor_bool,
            onvalue=True,
            offvalue=False,
            padx=2,
            width=20,
        ...
        self.lv_check = tk.Checkbutton(
            self.root,
            text="Senile Gwynevere",
            variable=self.use_lordvessel,
            onvalue=True,
            offvalue=False,
            padx=2,
            width=20,
        ...
        self.use_lordvessel.trace("w", lambda name, index, mode: self.update())
        self.lord_soul_check = tk.Checkbutton(
            self.root,
            text="Senile Primordial Serpents",
            variable=self.use_lord_souls,
            onvalue=True,
            offvalue=False,
            padx=2,
            width=20,
        ...
        self.setup_hover_events(
            self.lord_soul_check, {"use_lord_souls": None}, no_emph=True
        )
        ...
        self.export_button = tk.Button(
            ...
            padx=10,
            pady=10,
            command=self.export_to_gameparam,
        )
        self.export_button.grid(row=7, rowspan=3, column=4, padx=2, sticky="EW")

        self.cheat_button = tk.Button(
            self.root,
            text="Write Seed Info &\nCheatsheet / Hintsheet",
            command=self.export_seed_info,
        )
    ...
    def lift_sellout_area(self):
        ...
        self.msg_area.insert(
            "end",
            "If you are interested in supporting the author, please consider donating at\n\n               ",
        )
        self.msg_area.insert(
            "end", r"https://twitch.streamlabs.com/hotpocketremix/", "hyperlink"
        )
        self.msg_area.insert(
            "end",
        ...
        self.msg_area.tag_bind(
            "hyperlink", "<Button-1>", lambda _: self.hyperlink_click()
        )
    ...
    def update_desc_for_hover(self, hovered_parts, no_emph=False):
        ...
        for part in hovered_parts:
            if no_emph:
                ...
                overwrite_desc_specifiers[part] = (
                    current_state.desc_specifiers[part][0],
    ...
    def setup_hover_events(self, hover_object, hovered_parts, no_emph=False):
        hover_object.bind(
            "<Enter>", lambda _: self.update_desc_for_hover(hovered_parts, no_emph)
        )
        hover_object.bind(
            "<Leave>", lambda _: self.update_desc(respect_current_hover=False)
        )
        hover_object.bind(
            "<Return>", lambda _: self.update_desc(respect_current_hover=False)
        )
        hover_object.bind(
            "<space>", lambda _: self.update_desc(respect_current_hover=False)
        )
        hover_object.configure(
            command=lambda: self.update_desc_for_hover(hovered_parts, no_emph)
        )
    ...
    def randomize_data(self, chr_init_data):
        ...
        rng = random.Random()
        rng.seed(
            int(hashlib.sha256(self.seed_string.get().encode("utf-8")).hexdigest(), 16)
        )
        return (
            options,
            randomize_item_table.build_table(options, rng, chr_init_data),
            rng,
        )

    def get_new_seed(self):
        new_hex_seed = hashlib.sha256(
            str(self.seed_rng.random()).encode("utf-8")
        ).hexdigest()
        ...
        self.seed_entry.config(
            fg=self.entry_state.normal_color, font=self.entry_state.normal_font
        )
    ...
    def export_seed_info(self, use_randomized_data=None):
        if self.is_seed_empty():
            ...
            return
        if self.game_version.get() not in [
            rngopts.RandOptGameVersion.PTDE,
            rngopts.RandOptGameVersion.REMASTERED,
        ]:
        ...
        new_dir_name = "random-seed-" + datetime.datetime.today().strftime(
            "%Y-%m-%d--%H-%M-%S-%f"
        )
        ...
        if not use_randomized_data:
            ...
            self.msg_area.insert(
                "end", 'Click "Back" to begin again, or click "Quit" to exit.\n\n'
            )
